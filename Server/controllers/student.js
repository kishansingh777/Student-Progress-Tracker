...
export const studentLogin = async (req, res) => {
  ...
  try {
    ...
    if (!student) {
      return res
        .status(401)
        .json({ message: "Invalid rollNumber or studentClass" });
    }
...
export const addStudent = async (req, res) => {
  try {
    ...
    const { studentName, rollNumber, studentClass, studentClassId } = req.body;
    ...
    if (existingStudent) {
      return res
        ...
        .json({
          message:
            "A student with the same roll number already exists in this class",
        });
    }
    ...
    res
      ...
      .json({
        ...
        newStudent,
        studentMarks,
      }); // Return the saved student document as JSON
  } catch (error) {
    console.error(error);
...
export const addStudent = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: 'Student Added Successfully', savedStudent });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ message: 'Error adding student to database' });
  }
};
